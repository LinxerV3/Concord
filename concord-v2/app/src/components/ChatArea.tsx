...
const AUTO_IMG = /(https?:\/\/[^\s]+\.(?:png|jpe?g|gif|webp)(?:\?[^\s]*)?)/i;
...
export default function ChatArea(p: Props) {
  ...
  const onScroll = () => {
    ...
    setAtBottom(el.scrollTop + el.clientHeight > el.scrollHeight - 120);
  };
  const jumpToBottom = () => {
    const el = scrollRef.current; if (el) { el.scrollTop = el.scrollHeight; setAtBottom(true); }
  };
  ...
  const send = async () => {
    let text = input.trim();
    const staged = pendingImage && !pendingImage.uploading ? pendingImage.url : "";
    if ((!text && !staged) || !col) return;
    if (timedOut || channelLocked) return;
    text = censor(text);
    const slow = (channel?.slowMode || 0) * 1000;
    if (slow && !p.me.isAdmin && Date.now() - lastSentAt.current < slow) return;
    lastSentAt.current = Date.now();
    let imageUrl = staged;
    if (!imageUrl) { const m = text.match(AUTO_IMG); if (m) { imageUrl = m[1]; text = text.replace(m[1], "").trim(); } }
    setInput(""); setEmojiOpen(false); setPendingImage(null);
    const reply = replyingTo; setReplyingTo(null);
    const id = crypto.randomUUID();
    socketRef.current?.send({ type: "msg", id, text, isAdmin: !!p.me.isAdmin, imageUrl });
    await setDoc(doc(col, id), {
      uid: p.myUid, username: p.me.username, text, isAdmin: !!p.me.isAdmin,
      imageUrl: imageUrl || null, reactions: {}, replyTo: reply || null, ts: serverTimestamp(),
    });
  ...
  const onKeyDown = (e: React.KeyboardEvent) => {
    if (suggestions.length) {
      ...
      if (e.key === "Tab") { e.preventDefault(); acceptSuggestion(suggestions[acIndex]); return; }
      if (e.key === "Enter") { e.preventDefault(); acceptSuggestion(suggestions[acIndex]); return; }
    }
  ...
  const doUploadImage = async (file: File) => {
    setPendingImage({ url: URL.createObjectURL(file), uploading: true });
    try { const url = await uploadChatImage(p.myUid, file); setPendingImage({ url, uploading: false }); }
    catch (e: any) { toast(e.message || "Image upload failed — is Storage enabled?"); setPendingImage(null); }
  };
  const onImgInput = (e: React.ChangeEvent<HTMLInputElement>) => {
    const f = e.target.files?.[0]; if (f) doUploadImage(f); e.target.value = "";
  };
  const onPaste = (e: React.ClipboardEvent) => {
    const item = Array.from(e.clipboardData.items).find((i) => i.type.startsWith("image/"));
    if (item) { const f = item.getAsFile(); if (f) { e.preventDefault(); doUploadImage(f); } }
  };
  ...
  const saveEdit = async (m: Message) => {
    if (!col) return;
    const t = editText.trim(); setEditingId(null);
    ...
    <>
      <div className="main">
        ...
        <div className="chatHeader">
          ...
          {pinned.length > 0 && (
            <button className="iconBtn" title="Pinned" onClick={() => setShowPinned((s) => !s)}>📌 {pinned.length}</button>
          )}
            ...
            return (
              <div key={m.id} className={"msg " + (cont ? "cont" : "firstOfGroup") +
                ...
                <div className="msgBody">
                  ...
                  {!cont && (
                    <div className="msgHead">
                      <span className={"msgUser" + (u?.nameEffect === "rainbow" ? " rainbow" : "")}
                        ...
                        {m.system ? "SYSTEM" : (u?.displayName || m.username || u?.username || "unknown")}
                      </span>
                  ...
                  {m.imageUrl && (
                    <img className="msgImage" src={m.imageUrl} alt="" loading="lazy"
        ...
        {!atBottom && (
          <button className="jumpBtn" onClick={jumpToBottom}>↓ Jump to newest</button>
        )}
        <div className="composer">
          ...
          {pendingImage && (
            <div className="stagedImage">
              <img src={pendingImage.url} alt="" />
              {pendingImage.uploading && <div className="stagedSpin">Uploading…</div>}
              <button className="stagedX" onClick={() => setPendingImage(null)}>✕</button>
            </div>
          )}
          {timedOut ? (
            ...
            <div className="composerInner">
              <button className="emojiToggle" title="Add image / GIF" onClick={pickImage}>🖼️</button>
              <input ref={imgInputRef} type="file" accept="image/*" hidden onChange={onImgInput} />
              ...
              <input ref={inputRef} value={input} maxLength={1500}
                ...
                onChange={(e) => onType(e.target.value)} onKeyDown={onKeyDown} onPaste={onPaste} />
