...
export function SettingsModal({ me, myUid, onClose }: { me: UserDoc; myUid: string; onClose: () => void }) {
  const [tab, setTab] = useState<"profile" | "status" | "theme" | "appearance">("profile");
  ...
  return (
    <Modal onClose={onClose}>
      ...
      <h2>Settings</h2>
      <div className="tabs">
        {(["profile", "status", "theme", "appearance"] as const).map((t) => (
          <button key={t} className={"tab" + (tab === t ? " active" : "")} onClick={() => setTab(t)}>
            {{ profile: "Profile", status: "Status", theme: "Theme", appearance: "Appearance" }[t]}
          </button>
        ))}
      </div>
      {cropFile && (
