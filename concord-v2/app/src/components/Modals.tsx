...
import { PRESET_THEMES, DEFAULT_THEME_ID, applyTheme, resolveTheme, applyAppearance } from "../lib/theme";
import { uploadEmoji, uploadBlob } from "../lib/storage";
...
/* =========================================================
   Profile card
========================================================= */
export function ProfileModal({ uid, me, myUid, allUsers, servers, onClose, onMessage, onEdit }: {
  ...
  const joinOrder = Object.values(allUsers)
    .filter((x) => x.createdAt)
    .sort((a, b) => (a.createdAt!.seconds || 0) - (b.createdAt!.seconds || 0));
  const memberNum = joinOrder.findIndex((x) => x === u) + 1;
  const mutual = servers ? Object.values(servers).filter((sv) => !sv.isGlobal &&
    (u.joinedServers || []).includes(sv.id) && (me.joinedServers || []).includes(sv.id)) : [];
  const bannerStyle = u.bannerUrl
    ? { backgroundImage: `url(${u.bannerUrl})`, backgroundSize: "cover", backgroundPosition: "center" }
    : { background: u.banner || "#7c6cff" };
  return (
    <Modal onClose={onClose} raw>
      <div className="pCard pop">
        <div className="pBanner" style={bannerStyle} />
        <div className="pBody">
          <div className="pAvatar">{u.avatarUrl ? <img src={u.avatarUrl} alt="" /> : (u.avatarEmoji || "🙂")}</div>
          <div className={"pName" + (u.nameEffect === "rainbow" ? " rainbow" : "")} style={{ color: u.nameEffect === "rainbow" ? undefined : (u.accent || "var(--txt)") }}>
            {u.displayName || u.username} {u.isAdmin && <span className="badge">ADMIN</span>}
            {(u.badges || []).map((b) => BADGES[b] && (
              <span key={b} className="userBadge" title={BADGES[b].label} style={{ color: BADGES[b].color }}>{BADGES[b].emoji}</span>
            ))}
          </div>
          {u.displayName && <div className="pTag" style={{ marginTop: 0 }}>@{u.username}</div>}
          ...
          {u.activity && (
            <div className="pBox"><div className="lbl">DOING</div><div className="val">🎯 {u.activity}</div></div>
          )}
          <div className="pBox"><div className="lbl">ABOUT ME</div>
            <div className="val">{u.bio || <span className="pEmpty">Nothing here yet.</span>}</div>
          </div>
          {mutual.length > 0 && (
            <div className="pBox"><div className="lbl">MUTUAL SERVERS — {mutual.length}</div>
              <div className="val">{mutual.map((s) => (s.icon || "🟣") + " " + s.name).join("   ")}</div></div>
          )}
          {u.links && u.links.length > 0 && (
            <div className="pBox"><div className="lbl">LINKS</div>
              <div className="pLinks">
                {u.links.map((l, i) => (
                  <a key={i} href={l.url} target="_blank" rel="noreferrer" className="pLink">🔗 {l.label || l.url}</a>
                ))}
              </div>
            </div>
          )}
          <div className="pBox"><div className="lbl">MEMBER SINCE</div><div className="val">{created}{memberNum > 0 ? `  ·  member #${memberNum}` : ""}</div></div>
...
export function SettingsModal({ me, myUid, onClose }: { me: UserDoc; myUid: string; onClose: () => void }) {
  const [tab, setTab] = useState<"profile" | "status" | "theme" | "appearance" | "notifications" | "about">("profile");
  const [f, setF] = useState({
    username: me.username, displayName: me.displayName || "",
    avatarEmoji: me.avatarEmoji || "", avatarUrl: me.avatarUrl || "",
    pronouns: me.pronouns || "", banner: me.banner || "#7c6cff", bannerUrl: me.bannerUrl || "",
    accent: me.accent || "#e6e7ec", customStatus: me.customStatus || "", bio: me.bio || "",
    activity: me.activity || "",
  });
  const [links, setLinks] = useState<{ label: string; url: string }[]>(me.links || []);
  const [uploading, setUploading] = useState(false);
  const [uploadingBanner, setUploadingBanner] = useState(false);
  const [cropFile, setCropFile] = useState<{ file: File; kind: "avatar" | "banner" } | null>(null);
  ...
  const onBanner = (e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0]; if (!file) return;
    setCropFile({ file, kind: "banner" });
    e.target.value = "";
  };
  const onCropped = async (blob: Blob) => {
    const kind = cropFile!.kind; setCropFile(null);
    if (kind === "avatar") setUploading(true); else setUploadingBanner(true);
    try {
      const url = await uploadBlob(myUid, kind === "avatar" ? "avatars" : "banners", blob);
      set(kind === "avatar" ? "avatarUrl" : "bannerUrl", url);
      toast(kind === "avatar" ? "Picture set" : "Banner set");
    } catch (err: any) { toast(err.message || "Upload failed — is Storage enabled?"); }
    setUploading(false); setUploadingBanner(false);
  };
  ...
  const saveProfile = async () => {
    ...
    await updateDoc(doc(db, "users", myUid), {
      ...
      displayName: f.displayName.trim().slice(0, 24),
      avatarEmoji: f.avatarEmoji.trim() || "🙂", avatarUrl: f.avatarUrl.trim(),
      pronouns: f.pronouns.trim(), banner: f.banner, bannerUrl: f.bannerUrl.trim(), accent: f.accent,
      customStatus: f.customStatus.trim(), bio: f.bio.trim(), activity: f.activity.trim().slice(0,60),
      links: links.filter((l) => l.url.trim()).slice(0, 5),
    });
  ...
  return (
    <Modal onClose={onClose}>
      ...
      <h2>Settings</h2>
      <div className="tabs">
        {(["profile", "status", "theme", "appearance", "notifications", "about"] as const).map((t) => (
          <button key={t} className={"tab" + (tab === t ? " active" : "")} onClick={() => setTab(t)}>
            {{ profile: "Profile", status: "Status", theme: "Theme", appearance: "Appearance", notifications: "Notifications", about: "About" }[t]}
          </button>
        ))}
      </div>
      {cropFile && (
        <div className="cropOverlay">
          <CropEditor file={cropFile.file} shape={cropFile.kind === "avatar" ? "circle" : "rect"}
            outW={cropFile.kind === "avatar" ? 256 : 600} outH={cropFile.kind === "avatar" ? 256 : 200}
            boxW={cropFile.kind === "avatar" ? 260 : 320}
      ...
      {tab === "profile" && (
        <>
          <div className="bannerPreview" style={f.bannerUrl
            ? { backgroundImage: `url(${f.bannerUrl})`, backgroundSize: "cover", backgroundPosition: "center" }
            : { background: f.banner }}>
            <div className="bannerBtns">
              <button className="btn sm" onClick={() => bannerRef.current?.click()} disabled={uploadingBanner}>
                {uploadingBanner ? "Uploading…" : "🖼️ Banner image"}</button>
              {f.bannerUrl && <button className="btn sm ghost" onClick={() => set("bannerUrl", "")}>Remove</button>}
          ...
          <div className="row">
            <div className="field"><label>USERNAME (unique)</label>
            ...
            <div className="field"><label>DISPLAY NAME (optional)</label>
              <input value={f.displayName} maxLength={24} placeholder="shown instead of username"
                onChange={(e) => set("displayName", e.target.value)} /></div>
          </div>
          <div className="row">
          ...
          <div className="row">
            <div className="field"><label>BANNER COLOR (if no image)</label>
          ...
          <div className="field"><label>DOING (activity line)</label>
            <input value={f.activity} maxLength={60} placeholder="grinding ranked, watching JoJo…" onChange={(e) => set("activity", e.target.value)} /></div>
          <div className="field"><label>LINKS ({links.length}/5)</label>
            {links.map((l, i) => (
              <div className="row" key={i} style={{ marginBottom: 6 }}>
                <input placeholder="Label (e.g. YouTube)" value={l.label} maxLength={24}
                ...
                <button className="btn sm ghost" style={{ flex: "0 0 auto", width: "auto" }}
                  onClick={() => setLinks((ls) => ls.filter((_, j) => j !== i))}>✕</button>
              </div>
            ))}
            {links.length < 5 && (
              <button className="btn sm ghost" onClick={() => setLinks((ls) => [...ls, { label: "", url: "" }])}>＋ Add link</button>
            )}
          </div>
      ...
      {tab === "appearance" && <AppearanceTab me={me} myUid={myUid} />}
...
/* ---------- Theme tab ---------- */
/* ---------- Appearance tab ---------- */
/* ---------- Notifications tab (client-side prefs) ---------- */
function NotificationsTab() {
  const read = (k: string, d: boolean) => (localStorage.getItem(k) ?? String(d)) === "true";
  const [msgSound, setMsgSound] = useState(read("concord_sound_msg", false));
  const [mentionSound, setMentionSound] = useState(read("concord_sound_mention", true));
  const [flashTitle, setFlashTitle] = useState(read("concord_flash_title", true));
  const save = (k: string, v: boolean, set: (b: boolean) => void) => { localStorage.setItem(k, String(v)); set(v); };

  const Toggle = ({ on, onClick, label, hint }: { on: boolean; onClick: () => void; label: string; hint: string }) => (
    <div className="prefRow" onClick={onClick}>
      <div><div className="prefLabel">{label}</div><div className="prefHint">{hint}</div></div>
      <div className={"switch" + (on ? " on" : "")}><div className="knob" /></div>
    </div>
  );

  return (
    <>
      <p className="modalSub">These are saved on this device.</p>
      <Toggle on={msgSound} label="Message sounds" hint="Play a sound for every new message"
        onClick={() => save("concord_sound_msg", !msgSound, setMsgSound)} />
      <Toggle on={mentionSound} label="Mention pings" hint="Play a sound when someone @mentions you"
        onClick={() => save("concord_sound_mention", !mentionSound, setMentionSound)} />
      <Toggle on={flashTitle} label="Tab title alerts" hint="Flash the browser tab on new activity"
        onClick={() => save("concord_flash_title", !flashTitle, setFlashTitle)} />
    </>
  );
}
...
function AboutTab() {
  return (
    <div className="aboutPane">
      <div className="aboutMark">⌁</div>
      <div className="aboutName">Concord</div>
      <div className="aboutVer">version 2.0 · built by Michael</div>
      <p className="modalSub" style={{ textAlign: "center", marginTop: 12 }}>
        A real-time chat platform — servers, DMs, custom profiles, themes, emoji, and more.
        Powered by Cloudflare Durable Objects + Firebase.
      </p>
      <div className="aboutStats">
        <div className="aboutStat"><b>Real-time</b><span>WebSocket messaging</span></div>
        <div className="aboutStat"><b>Yours</b><span>self-hosted &amp; open</span></div>
        <div className="aboutStat"><b>Sleek</b><span>neon glass UI</span></div>
      </div>
    </div>
  );
}

function AppearanceTab({ me, myUid }: { me: UserDoc; myUid: string }) {
  const a = (me as any).appearance || {};
  const [density, setDensity] = useState<"cozy" | "compact">(a.density || "cozy");
  const [fontScale, setFontScale] = useState<number>(a.fontScale ?? 1);
  const [hue, setHue] = useState<number>(a.hue ?? 0);

  const live = (d: string, fs: number, h: number) => applyAppearance({ density: d as any, fontScale: fs, hue: h });
  const save = async () => {
    await updateDoc(doc(db, "users", myUid), { appearance: { density, fontScale, hue } });
    toast("Appearance saved");
  };

  return (
    <>
      <p className="modalSub">Tune how the app looks and feels. Applies live, saves to your account.</p>
      <div className="field"><label>MESSAGE DENSITY</label>
        <div className="segRow">
          <button className={"segBtn" + (density === "cozy" ? " active" : "")}
            onClick={() => { setDensity("cozy"); live("cozy", fontScale, hue); }}>Cozy</button>
          <button className={"segBtn" + (density === "compact" ? " active" : "")}
            onClick={() => { setDensity("compact"); live("compact", fontScale, hue); }}>Compact</button>
        </div>
      </div>
      <div className="field"><label>TEXT SIZE — {Math.round(fontScale * 100)}%</label>
        <input type="range" min={0.85} max={1.25} step={0.01} value={fontScale}
          onChange={(e) => { const v = Number(e.target.value); setFontScale(v); live(density, v, hue); }} /></div>
      <div className="field"><label>ACCENT HUE SHIFT — {hue}&deg;</label>
        <input type="range" min={-180} max={180} step={1} value={hue}
          onChange={(e) => { const v = Number(e.target.value); setHue(v); live(density, fontScale, v); }} /></div>
      <button className="btn" onClick={save}>Save appearance</button>
    </>
  );
}
