...
import { auth, db, ADMIN_TRIGGER, EMAIL_DOMAIN, GLOBAL_ID, isOnline, statusLabel } from "./lib/firebase";
import { applyTheme, DEFAULT_THEME_ID, applyAppearance } from "./lib/theme";
import { confetti, disco, emojiRain, flip, shake, toast } from "./lib/fx";
import { setCustomEmojis } from "./lib/emoji";
...
export default function App() {
  ...
  const [customEmojis, setCustomEmojisState] = useState<Record<string, string>>({});
  const [bannedWords, setBannedWords] = useState<string[]>([]);
  const [view, setView] = useState<View>({ type: "server", sid: GLOBAL_ID, cid: null });
  const [announcement, setAnnouncement] = useState<string | null>(null);
  const [announceTs, setAnnounceTs] = useState(0);
  ...
  useEffect(() => {
    ...
    return onSnapshot(doc(db, "users", myUid), (s) => {
      ...
      setMe(data);
      applyTheme(data.theme ?? DEFAULT_THEME_ID);
      applyAppearance((data as any).appearance || {});
    });
  }, [myUid]);

  /* ---------- Esc closes any open modal ---------- */
  useEffect(() => {
    const h = (e: KeyboardEvent) => { if (e.key === "Escape") setModal({ kind: "none" }); };
  ...
  useEffect(() => {
    ...
    const un4 = onSnapshot(doc(db, "system", "announcement"), (s) => {
      ...
      const dismissed = Number(localStorage.getItem("concord_announce_dismissed") || 0);
      if (t > dismissed) { setAnnounceTs(t); setAnnouncement(a.text); }
    ...
    const un6 = onSnapshot(collection(db, "customEmojis"), (snap) => {
      const map: Record<string, string> = {};
      snap.forEach((d) => { const e = d.data() as any; if (e.name && e.url) map[e.name] = e.url; });
      setCustomEmojis(map);
      setCustomEmojisState(map);
    });
    const un7 = onSnapshot(doc(db, "system", "config"), (s) => {
      const c = s.data() as any;
      setBannedWords(Array.isArray(c?.bannedWords) ? c.bannedWords : []);
    });
    return () => { un1(); un2(); un3(); un4(); un5(); un6(); un7(); clearInterval(hb); };
  }, [myUid]);
  ...
  // splash covers the very first load; reveal app when auth + (if logged in) my doc are ready
  if (authReady && (!myUid || me) && !bootReady) setTimeout(() => setBootReady(true), 300);

  const splash = <Splash done={bootReady} />;
  if (!authReady) return splash;
  if (!myUid) return <>{splash}<AuthScreen /></>;
  if (!me) return splash;
  ...
  const openDM = async (otherUid: string) => {
    ...
    // Navigate first so clicking Message ALWAYS opens the conversation,
    // even if the Firestore write is slow or blocked.
    setView({ type: "home", dmId, otherUid });
    document.body.classList.remove("sideOpen");
    // Ensure the DM doc exists (non-blocking; failures won't break navigation).
    try {
      const ref = doc(db, "dms", dmId);
      const s = await getDoc(ref);
      if (!s.exists()) await setDoc(ref, { users: [myUid, otherUid], lastTs: serverTimestamp() });
    } catch (e) {
      console.warn("Could not create DM doc (check Firestore rules):", e);
    }
  };
  ...
  return (
    <>{splash}<div className="app">
      ...
      <div className="sidebar">
        ...
        <div className="userBar">
          ...
          <div className="userBarInfo" onClick={() => setModal({ kind: "settings" })} title="Edit profile">
            ...
            <div className="s">{statusLabel(me.statusMode) || me.customStatus || "Online"}</div>
          </div>
          {me.isAdmin && (
      ...
      <ChatArea
        ...
        dismissAnnouncement={() => {
          if (announceTs) localStorage.setItem("concord_announce_dismissed", String(announceTs));
      ...
      {/* ===== modals ===== */}
      {modal.kind === "profile" && (
        <ProfileModal uid={modal.uid} me={me} myUid={myUid} allUsers={allUsers} servers={servers}
